...
[![NPM License](https://img.shields.io/npm/l/multiple-cucumber-html-reporter-plugin.svg)](https://github.com/wswebcreation/multiple-cucumber-html-reporter-plugin/blob/master/LICENSE)
...
- Tnx to [yoghi](https://github.com/yoghi) it is now possible: 
    ...
    - to override the style by adding your own stylesheet, See [`overrideStyle`](#overrideStyle) for more info 
    - to add some custom styles by adding your own style sheet that will be loaded after the default stylessheet See [`customstyle`](#customstyle) for more info
...
This expects the durations in the report to be in milliseconds, which might result in incorrect durations when using a version of Cucumber that does not report in milliseconds (like v.1), see all examples with `V1` behind the name of the feature.
