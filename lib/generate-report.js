...
function generateReport(options) {
  ...
  const showFeatureFolder = options.showFeatureFolder || false;
  ...
  let suite = {
    ...
    showFeatureFolder: showFeatureFolder,
    browser: 0,
    name: '',
    version: 'version',
    time: new Date(),
...
=====================================================================================\n`));
  ...
  function _parseFeatures(suite) {
    suite.features.forEach(feature => {
      ...
      var uriSegments = feature.uri.split('\\');
      feature.folder = uriSegments[uriSegments.length - 2] || '-';
