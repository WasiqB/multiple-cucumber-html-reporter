...
=====================================================================================\n`)
				...
				step.embeddings.forEach((embedding, embeddingIndex) => {
					...
					if (
						...
						step.html = (step.html ? step.html : []).concat([
							_isBase64(embedding.data) ? Base64.decode(embedding.data) : embedding.data
						]);
					} else if (
	 ...
	 * @param {string} customFileName - when provided, this template will take precedence
	 * @return {*} Content of the file
	 * @private
	 */
	function _readTemplateFile(fileName, customFileName) {
		if (customFileName) {
			try {
				fs.accessSync(customFileName, fs.constants.R_OK);
				return fs.readFileSync(customFileName, 'utf-8');
			} catch (error) {
				throw new Error(`Cannot read the custom template file '${customFileName}'`);
			}
		}
		...
		fs.writeFileSync(
			...
			_.template(_readTemplateFile(FEATURES_OVERVIEW_INDEX_TEMPLATE))({
				...
				featuresOverview: _.template(
					_readTemplateFile(FEATURES_OVERVIEW_TEMPLATE, suite.customTemplate.featuresOverview)
				)({
					suite: suite,
					_: _
				}),
