...
function generateReport(options) {
	...
	const plainDescription = options.plainDescription || false;
	const style = options.overrideStyle || REPORT_STYLESHEET;
	const customStyle = options.customStyle;
	const disableLog = options.disableLog || false;
	const openReportInBrowser = options.openReportInBrowser || false;
	const reportName = options.reportName || DEFAULT_REPORT_NAME;
	const reportPath = path.resolve(process.cwd(), options.reportPath);
	const saveCollectedJSON = options.saveCollectedJSON || false;
