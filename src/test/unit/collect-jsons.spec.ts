...
describe('collect-jsons.js', () => {
    describe('Happy flows', () => {
        ...
        it('should return an output from the merged found json files and add the provided metadata', () => {
            ...
            })).toEqual(jsonFile.readFileSync(path.resolve(process.cwd(), './src/test/unit/data/output/provided-metadata.json')));
        });
    ...
    describe('failures', () => {
        ...
        it('should print a console message when no json files could be found', () => {
            ...
            expect(console.log).toHaveBeenCalledWith('\x1b[33m%s\x1b[0m', `WARNING: No JSON files found in './src/test/unit/data/no-jsons'. NO REPORT CAN BE CREATED!`)
        });
