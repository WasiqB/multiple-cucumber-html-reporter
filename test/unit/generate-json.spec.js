...
describe('generate-report.js', () => {
	describe('Happy flows', () => {
		...
		it('should create a report from the merged found json files with custom template', () => {
			fs.removeSync(REPORT_PATH);
			multiCucumberHTMLReporter.generate({
				jsonDir: './test/unit/data/json',
				reportPath: REPORT_PATH,
			...
			expect(fs.readFileSync(`${path.join(process.cwd(), REPORT_PATH, 'index.html')}`)).toContain('Custom Column');
			expect(fs.readFileSync(`${path.join(process.cwd(), REPORT_PATH, 'index.html')}`)).toContain('Custom Content');
		});
		...
		it('should create a report from the merged found json files with custom metadata and custom template', () => {
			fs.removeSync(REPORT_PATH);
			multiCucumberHTMLReporter.generate({
				jsonDir: './test/unit/data/custom-metadata-json/',
				reportPath: REPORT_PATH,
	...
	describe('failures', () => {
		...
		it('should throw an error when custom template file does not exist', () => {
			fs.removeSync(REPORT_PATH);
			expect(() =>
				multiCucumberHTMLReporter.generate({
					jsonDir: './test/unit/data/json',
					...
					displayDuration: true,
					customTemplate: {
						featuresOverview: 'foobar'
					}
				})
			).toThrowError("Cannot read the custom template file 'foobar'");
		});
	});
});
