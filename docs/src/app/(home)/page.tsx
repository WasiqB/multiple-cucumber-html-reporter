...
export default function HomePage() {
  return (
    <main className="flex flex-col gap-24 pb-20 overflow-x-hidden">
      {/* Hero Section */}
        ...
        <div className="container mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <div className="flex flex-wrap gap-4 mt-4">
              <Link href="/docs" className={cn(buttonVariants({ size: 'default' }), "bg-emerald-600 shadow-lg shadow-emerald-600/20 hover:bg-emerald-700 text-white rounded-xl px-8 h-12 text-base font-bold transition-all hover:scale-[1.02] active:scale-[0.98]")}>
                Try It Now
              </Link>
              <Link href="https://github.com/WasiqB/multiple-cucumber-html-reporter" className={cn(buttonVariants({ variant: 'ghost', size: 'default' }), "bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl px-8 h-12 text-base font-semibold group")}>
        ...
        <div className="bg-zinc-950 rounded-3xl border border-zinc-800 p-8 md:p-12 shadow-2xl relative overflow-hidden group">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="flex flex-col gap-6">
              ...
              <Link href="/docs" className="text-emerald-400 font-bold flex items-center group-hover:gap-2 transition-all">
      ...
      <section className="border-t border-zinc-200 dark:border-zinc-800 pt-20 px-6">
        <div className="container mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-x-16 gap-y-10 opacity-50 grayscale contrast-125 dark:invert">
            ...
            <span className="text-2xl font-medium uppercase tracking-widest underline decoration-wavy decoration-emerald-500">VELOCITY</span>
            <span className="text-2xl font-mono uppercase font-bold tracking-widest border-2 border-zinc-900 dark:border-zinc-100 p-1">STACKWARE</span>
            <span className="text-2xl font-sans tracking-tight leading-4">Cloud<span className="font-bold underline decoration-4 decoration-emerald-500">Pave</span></span>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-sm text-zinc-500">
              © 2024 Precision Curator. Open source under MIT License.
            </p>
          </div>
          <div className="flex flex-wrap gap-8 text-sm font-medium text-zinc-600 dark:text-zinc-400">
            ...
            <Link href="#" className="hover:text-emerald-500 transition-colors">Changelog</Link>
            <Link href="#" className="hover:text-emerald-500 transition-colors">Status</Link>
            <Link href="#" className="hover:text-emerald-500 transition-colors">Sponsor</Link>
            <Link href="#" className="hover:text-emerald-500 transition-colors">Contributing</Link>
          </div>
        </div>
      </footer>
    </main>
...
function SupportCard({ icon, title, description, buttonText, href, color }: { icon: React.ReactNode, title: string, description: string, buttonText: string, href: string, color: string }) {
  const colorMap = {
