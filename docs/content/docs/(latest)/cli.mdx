...
Check out the [installation step](/docs/installation) to install the CLI globally.
...
<TypeTable
  type={{
    ...
    '--email': {
      description: 'Generates a standalone emailable HTML report summary (email-report.html)',
      type: 'boolean',
      typeDescription: 'boolean'
    },
    '--version': {
...
To generate a standalone emailable HTML report summary, you can use the `--email` flag with the `mchr` command:

```shell
mchr --email
```

<Callout title='Alternaate option' type='idea'>
You can skip the `--email` option by adding this option in the options file.
</Callout>

This will generate a standalone emailable HTML report summary in the `./<report-path>/email-report.html` file based on your config file.
